    ...
    with slim.arg_scope([slim.conv2d, bottleneck,
      ...
      with slim.arg_scope([slim.batch_norm], is_training=is_training):
        ...
        if global_pool:
          ...
          net = tf.reduce_mean(net, [1, 2], name='pool5', keep_dims=True) #32x1x1x2048
        pdb.set_trace()
        if num_classes is not None:
          net = slim.conv2d(net, num_classes, [1, 1], activation_fn=None,
                            normalizer_fn=None, scope='logits') #32x1x1x21
        if spatial_squeeze:
...
def resnet_v2_50_multiple(inputs,
                 num_classes=None,
                 is_training=True,
                 global_pool=True,
                 output_stride=None,
  ...
  blocks = [
      ...
      resnet_utils.Block(
          'block4', bottleneck, [(2048, 512, 1)] * 3)]
  return resnet_v2_multiple(inputs, blocks, num_classes, is_training=is_training,
                   global_pool=global_pool, output_stride=output_stride,
                   include_root_block=True, spatial_squeeze=spatial_squeeze,
                   reuse=reuse, scope=scope)
resnet_v2_50_multiple.default_image_size = resnet_v2.default_image_size
