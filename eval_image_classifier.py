...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    labels_0 = tf.squeeze(labels_0)
    labels_1 = tf.squeeze(labels_1)
    labels_2 = tf.squeeze(labels_2)
    # predictions = tf.stack([predictions_0, predictions_1, predictions_2], axis=1)
    predictions = predictions_0 + predictions_1 + predictions_2
    # labels = tf.stack([labels_0, labels_1, labels_2], axis=1)
    labels = labels_0 + labels_1 + labels_2
    # pdb.set_trace()

    # Define the metrics:
    names_to_values, names_to_updates = slim.metrics.aggregate_metric_map({
        'Accuracy_0': slim.metrics.streaming_accuracy(predictions_0, labels_0),
        'Accuracy_1': slim.metrics.streaming_accuracy(predictions_1, labels_1),
        'Accuracy_2': slim.metrics.streaming_accuracy(predictions_2, labels_2),
        'Accuracy_all': metric_utils.streaming_accuracy(predictions, labels),
